...
data = load('G:\EUCLID-plasticity-main\FEM_data\20240110T165055_DATA_FYS_1_HardMix_plate_elliptic_holes.mat');
data = data.data;
...
% Nonlinear function 
fun_vec = @(parameters) objective_vec_HardMixVAF_mex11_utc5( ...
        ... % (hyper-)parameters
        parameters, ...
        in.q_case, ...
        in.q, ...
...
nburn = (0.25)*nSamples*p; %------ 30% burn in

verbosity = true; % If verbosity is set to 'true' it will print the MCMC interations
...
sig_prop_vect = [0.2,ones(1,6).*0.01,[1,0.01,1]];
